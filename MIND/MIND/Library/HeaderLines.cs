    ...
    class HeaderLines: LinesText
    {
        ...
        public HeaderLines(string s, int st) : base(st)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                if (formateds[i].Count > 1)
                {
                    ...
                    for (int j = 0; j < matrix.Length; j++)
                    {
                        ...
                        switch (Math.Abs(current))
                        {
                            case 1:
                                {
                                    inLines.Add(new Link(formateds[i].GetRange(j, k - j + 1), sized[count]));
                            ...
                            case 2:
                                {
                                    inLines.Add(new ImageText(formateds[i].GetRange(j, k - j + 1), null, sized[count]));
                            ...
                            case 4:
                                {
                                    inLines.Add(new SimpleInLineText(formateds[i].GetRange(j, k - j + 1), sized[count]));
                                    inLines[inLines.Count - 1].startString = y;
                                    inLines[inLines.Count - 1].startX = x;
                                    x += inLines[inLines.Count - 1].value.Width;
                                    break;
                ...
                {
                    inLines.Add(new SimpleInLineText(formateds[i], sized[count]));
                    inLines[inLines.Count - 1].startString = y;
                    inLines[inLines.Count - 1].startX = x;
                    x += inLines[inLines.Count - 1].value.Width;
                }
